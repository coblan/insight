...
from user_admin.models import EmployeeModel
...
class TaskModel(models.Model):
    ...
    owner=models.ForeignKey(EmployeeModel,verbose_name='从属于',null=True,blank=True)
...
class WorkModel(models.Model):
    ...
    worker=models.ForeignKey(EmployeeModel,verbose_name='worker',null=True,blank=True)
    ...
    manager=models.ForeignKey(EmployeeModel,verbose_name='管理者',null=True,blank=True,related_name='managed_works')
    short_desp=models.CharField('工作简介',max_length=200,blank=True)
    long_desp=models.TextField('详细介绍',blank=True)
    
    def __unicode__(self):
        return unicode(self.task) + '_' + unicode(self.worker)